...
float StartMenu::lastX = 0.0f;
float StartMenu::lastY = 0.0f;
Camera2 StartMenu::camera = Camera2();

StartMenu::StartMenu()
{
}

/*void StartMenu::mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
...
void StartMenu::Init() //defines what shader to use
{
	//Background color
	glClearColor(255.0f, 200.0f, 0.0f, 0.0f); //4 parameters (RGBA)
	...
	//for main menu
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("Menu", (1.f, 1.f, 1.f), 1.f, 1.f, 0.f);
	meshList[GEO_QUAD]->textureID = LoadTGA("Image//play.tga");
...
{
	if (Application::IsKeyPressed('S')) //select instructions
	{
		meshList[GEO_QUAD]->textureID = inst;
		selectPlay = false;
	}
	if (Application::IsKeyPressed('W')) //select play
	{
		meshList[GEO_QUAD]->textureID = play;
		selectPlay = true;
	}
	...
	//camera.Update(dt); //fixed camera for menu
}
void StartMenu::Render()
{
	...
	// main menu
	modelStack.PushMatrix();
	modelStack.Scale(15, 15, 10);
	RenderMesh(meshList[GEO_QUAD], false);
	modelStack.PopMatrix();
}

void StartMenu::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void StartMenu::RenderSkybox()
{

}

void StartMenu::RenderButton(int geo_circle, int geo_cylinder)
{
	//<----Button circle---->
...
void StartMenu::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void StartMenu::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
...
void StartMenu::DrawHUD(Mesh* mesh, Color color, bool enableLight, float size, float x, float y)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
